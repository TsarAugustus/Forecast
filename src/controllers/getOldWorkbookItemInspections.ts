...
function getOldWorkbookItemInspections(item) {
	...
	if(item.UNIT && item.UNIT !== 'UNIT') {
		for(let inspection in item.INSPECTION) {
			let inspectionLetter: string;

			if(item.INSPECTION[inspection] === 'V') inspectionLetter = 'V'
			else if(item.INSPECTION[inspection] === 'I') inspectionLetter = 'I'
			else if(item.INSPECTION[inspection] === 'K') inspectionLetter = 'K'
			else if(item.INSPECTION[inspection] === 'P') inspectionLetter = 'P'
			else if(item.INSPECTION[inspection] === 'T') inspectionLetter = 'T'
			else if(item.INSPECTION[inspection] === 'U') inspectionLetter = 'UC'
			else if(item.INSPECTION[inspection] === 'C') inspectionLetter = 'UC'
			else if(item.INSPECTION[inspection] === 'W') inspectionLetter = 'WF'
			else if(item.INSPECTION[inspection] === 'F') inspectionLetter = 'WF'
			else if(item.INSPECTION[inspection] === 'L') inspectionLetter = 'L'
			else continue

			let spec = item['TANK SPEC'].toString();

			if(spec === 'HOSES' || spec === 'Hose' || spec === 'TANK SPEC') continue
