	...
	totalWorkbooks.forEach(workbookItem => {
			...
			employeeList.forEach(employee => {
				...
				if(thisSpec !== undefined && employeeName !== undefined) {
					...
					for(let letter = 0; letter < thisInspection.length; letter++) {
						...
						if(thisInspection[letter] === 'U' || thisInspection[letter] === 'C') thisLetter = 'UC';
						if(thisInspection[letter] === 'W' || thisInspection[letter] === 'F') thisLetter = 'WF';
		...
		if(workbookItem['Additional Personnel'] && workbookItem['Additional Personnel'] !== 'N/A') {
			...
			employeeList.forEach(employee => {
				...
				if(workbookItem.Visual !== 'N/A') thisInspection += 'V'
				if(workbookItem.Internal !== 'N/A') thisInspection += 'I'
				if(workbookItem.Leakage !== 'N/A') thisInspection += 'K'
				if(workbookItem.Pressure !== 'N/A') thisInspection += 'P'
				if(workbookItem.Thickness !== 'N/A') thisInspection += 'T'
				if(workbookItem.Lining !== 'N/A') thisInspection += 'L'
				if(workbookItem['Upper Coupler'] !== 'N/A') thisInspection += 'UC'
				if(workbookItem['Wet Fluorescent'] !== 'N/A') thisInspection += 'WF'
				...
				if(thisSpec !== undefined && employeeName !== undefined) {
					...
					for(let letter = 0; letter<thisInspection.length; letter++) {
						...
						if(thisLetter === 'U' || thisLetter === 'C') thisLetter = 'UC';
						if(thisLetter === 'W' || thisLetter === 'F') thisLetter = 'WF';
...
function getEmployeeName(string) {
	...
	if(string.includes('Dax')) nameToReturn = 'Dax Poitras'
	if(string.includes('Patrick')) nameToReturn = 'Patrick Trinh'
	if(string.includes('Carson')) nameToReturn = 'Carson Gray'
	if(string.includes('Paul')) nameToReturn = 'Paul Radacz'
	if(string.includes('Ryan')) nameToReturn = 'Ryan Spelling'
	if(string.includes('Cole')) nameToReturn = 'Cole Kaufman'
	if(string.includes('Jun')) nameToReturn = 'Jun Chae'
	if(string.includes('Tyler')) nameToReturn = 'Tyler Vanderwall'
	if(string.includes('Gurj')) nameToReturn = 'Gurjinder Biln'
	if(string.includes('Al')) nameToReturn = 'Al Smith'

	return nameToReturn;
}

function getTankSpec(potentialSpec) {
	let specToReturn: number;
	potentialSpec = potentialSpec.toString()
	if(potentialSpec && potentialSpec === 306 || potentialSpec === 406) specToReturn = 406
	if(potentialSpec && potentialSpec === 307 || potentialSpec === 407) specToReturn = 407
	if(potentialSpec && potentialSpec === 330 || potentialSpec === 331) specToReturn = 331
	if(potentialSpec === 412) specToReturn = 407
	if(!specToReturn && potentialSpec !== 'Hose') {
		if(potentialSpec && potentialSpec.includes('306') || potentialSpec.includes('406')) specToReturn = 406
		if(potentialSpec && potentialSpec.includes('307') || potentialSpec.includes('407')) specToReturn = 407
		if(potentialSpec && potentialSpec.includes('330') || potentialSpec.includes('331')) specToReturn = 331
	}
